...
**Objective:** Add the tooling to see *what's inside* each chunk, then configure Vite to split strategically.
...
**Objective:** Quantify JS payload per route *before* splitting (baseline vs. after).
...
for (const [key, value] of Object.entries(manifest)) {
  ...
  routes[routeName] = { chunks: allChunks.size, totalSize, files: sizes.sort((a, b) => b.size - a.size) };
}
...
**Objective:** For routes that don't *always* need SDK (e.g., home page), lazy-load it.
