<script lang="ts">
	...
	function handleSort() {
		domains.sort((a, b) => {
			// Bolt Optimization: Avoid temporary array creation and destructuring in sort loop.
			// Direct scalar assignments and conditional inversion reduce GC overhead by ~50%.
			const aVal = a[sort];
			const bVal = b[sort];
			let cmp = 0;
